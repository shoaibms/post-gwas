...
Execute each stage in order from the project root. Each stage has a dedicated `REPRODUCE_*.md` guide with full commands, arguments, and expected outputs. The guides are the authoritative execution reference; the summary below shows stage dependencies only.

```
01  Preprocessing       → see REPRODUCE_01_preprocessing.md
02  Transformer (ECT)   → see REPRODUCE_02_transformer.md
...
*Files in `output/` subdirectories. See [Data_note.md](data/Data_note.md) for provenance.*
...
> **Note:** We do not redistribute raw data. See [Data_note.md](data/Data_note.md) for download instructions.
...
*Complete frozen dependencies in `requirements.txt`*
...
## 🔭 Outlook & Downstream Applications

> **Note:** The applications below are proposed downstream uses based on the findings of this study. They represent hypotheses and translational directions that have not been experimentally validated here; the manuscript frames them as outlook.
...
### What Surprised Us
❗ **Mechanism ≠ Prediction:** Strong regulatory signals, modest phenotypic gain (ΔR² = 0.075; 95% CI −0.003 to 0.154; p = 0.08)  
→ *Possible explanations (not directly tested):* Polygenic buffering, trans-regulatory compensation, network degeneracy
